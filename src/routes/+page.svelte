  ...
  <div class="card recommended-workout">
    <div class="card-header">
      <div>
        ...
        <p class="card-subtitle">Week {data.mockData.currentWeek}, Day {data.mockData.currentDay}</p>
      </div>
    ...
    <p class="workout-description">
      Do one pull-up, then two, then three, etc. until you reach your maximum. Then work your way back down to one.
    </p>
    
    <button onclick={startWorkout} class="start-button">
      Start Workout
    </button>

    <button onclick={skipWorkout} class="skip-button">
      Skip Workout
    </button>
  </div>
        ...
        <div class="week-card">
          ...
          <div class="days-grid">
            {#each Array(5) as _, dayIndex}
              {@const isCompleted = weekIndex < data.mockData.currentWeek - 1 || (weekIndex === data.mockData.currentWeek - 1 && dayIndex < data.mockData.currentDay - 1)}
              {@const isCurrent = weekIndex === data.mockData.currentWeek - 1 && dayIndex === data.mockData.currentDay - 1}
